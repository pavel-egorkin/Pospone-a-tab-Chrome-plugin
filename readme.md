...
- **Notifications** (click opens Sleeping Tabs list) and **toolbar badge** with total upcoming count.
- **Configurable Keyboard shortcuts** for power use.
- Do not work in Incognito mode
- Manifest V3, minimal permissions, privacy‑first.

> **Design note**: Popup uses text‑only tiles; in the Sleeping Tabs window the **floating +** button creates a new to‑do tab.
...
### Preset semantics
- 1 Minute: now + 1 minute (for quick testing).
...
- Recurrences can be **Deleted** from the Sleeping Tabs list. Pause/Resume and Edit are not supported.
...
- When a tab wakes: desktop notification; clicking the notification opens the Sleeping Tabs list.
...
- **Danger zone**: Delete all snoozes and recurrences (confirmation dialog; no undo)
...
- On alarm: validate URL, open in the last-active normal window; if none, create a new normal window; open as a background tab (not focused). Remove the item immediately upon open. If opening fails, keep it and retry on next browser launch.
...
```json
{
  "permissions": ["tabs", "storage", "alarms", "notifications", "activeTab"],
...
- Basic keyboard support in popup (Tab focuses items; `Enter` confirms).
